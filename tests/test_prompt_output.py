...
class PromptTester:
    ...
    async def _run_technical_analysis_only(self):
        ...
        try:
            result = None
            async for chunk_result in technical_analysis_agent_stream(self.state):
                for key, value in chunk_result.items():
                    if key == 'analysis_results':
                        # 合并analysis_results而不是替换，保留data_collector信息
                        current_analysis = self.state.get("analysis_results", {})
                        current_analysis.update(value)
                        self.state[key] = current_analysis
                    else:
                        self.state[key] = value
                    # 保存最后的技术分析结果
    ...
    async def _run_all_analyses(self):
        ...
        async def run_fundamental_analysis():
            ...
            try:
                result = None
                async for chunk_result in fundamental_analysis_agent_stream(self.state):
                    for key, value in chunk_result.items():
                        if key == 'analysis_results':
                            # 合并analysis_results而不是替换，保留data_collector信息
                            current_analysis = self.state.get("analysis_results", {})
        ...
        async def run_news_sentiment_analysis():
            ...
            try:
                result = None
                async for chunk_result in news_sentiment_analysis_agent_stream(self.state):
                    for key, value in chunk_result.items():
                        if key == 'analysis_results':
                            # 合并analysis_results而不是替换，保留data_collector信息
                            current_analysis = self.state.get("analysis_results", {})
        ...
        # 整合分析结果到state中
        analysis_results = self.state.get("analysis_results", {})
        for agent_type, result in analysis_results_list:
            if isinstance((agent_type, result), tuple) and result is not None:
