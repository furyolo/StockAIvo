#!/usr/bin/env python3
"""
删除stock_prices_hourly表的自动化脚本

本脚本安全删除stock_prices_hourly表及其相关索引和约束。
小时线功能已被10分钟线取代，因此删除相关数据库结构。

使用方法:
    python drop_hourly_table.py
...
回滚方案:
    ...
    CREATE UNIQUE INDEX uk_hourly_ticker_hour ON stock_prices_hourly (ticker, hour_timestamp);
    CREATE INDEX idx_hourly_ticker ON stock_prices_hourly (ticker);
    CREATE INDEX idx_hourly_timestamp ON stock_prices_hourly (hour_timestamp);
    CREATE INDEX idx_hourly_ticker_timestamp ON stock_prices_hourly (ticker, hour_timestamp);
"""
        ...
        with engine.connect() as conn:
            check_table_sql = """
                SELECT EXISTS (
                    ...
                    AND table_name = 'stock_prices_hourly'
                )
            """
...
def drop_indexes_concurrently(engine):
    ...
    indexes_to_drop = [
        'idx_hourly_ticker',
        'idx_hourly_timestamp', 
        'idx_hourly_ticker_timestamp'
    ]
            ...
            try:
                ...
                check_index_sql = f"""
                    SELECT EXISTS (
                        ...
                        WHERE tablename = 'stock_prices_hourly' 
...
def drop_hourly_table():
    """删除stock_prices_hourly表及其相关索引和约束"""
    ...
    try:
        ...
        logger.info("开始删除stock_prices_hourly表...")
        
        # 检查表是否存在
        if not check_table_exists(engine):
            logger.info("stock_prices_hourly表不存在，无需删除")
            return True
        ...
        with engine.connect() as conn:
            stats_sql = """
                SELECT 
                    ...
                    pg_size_pretty(pg_total_relation_size('stock_prices_hourly')) as table_size
        ...
        with engine.connect() as conn:
            ...
            try:
                # 删除唯一约束
                logger.info("步骤2: 删除唯一约束...")
                drop_constraint_sql = """
                    ALTER TABLE stock_prices_hourly 
                    DROP CONSTRAINT IF EXISTS uk_hourly_ticker_hour
                """
                conn.execute(text(drop_constraint_sql))
                logger.info("唯一约束 uk_hourly_ticker_hour 删除成功")
                
                # 删除表
                logger.info("步骤3: 删除表...")
                drop_table_sql = "DROP TABLE IF EXISTS stock_prices_hourly CASCADE"
                conn.execute(text(drop_table_sql))
                logger.info("表 stock_prices_hourly 删除成功")
                ...
                # 回滚事务
                trans.rollback()
                logger.error(f"删除表或约束时出错: {e}")
                raise
        
        # 3. 验证删除结果
        logger.info("步骤4: 验证删除结果...")
        verify_deletion(engine)
        
        logger.info("stock_prices_hourly表及相关结构删除完成！")
        return True
...
def verify_deletion(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            if table_exists:
                logger.error("验证失败: stock_prices_hourly表仍然存在")
                return False
            else:
                logger.info("✓ 验证成功: stock_prices_hourly表已删除")
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("stock_prices_hourly表删除脚本")
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--force":
        ...
        confirm = input("确认要删除stock_prices_hourly表及其所有数据吗？(输入 'yes' 确认): ")
        if confirm.lower() != 'yes':
            logger.info("操作已取消")
            sys.exit(0)
    
    # 执行删除
    success = drop_hourly_table()
