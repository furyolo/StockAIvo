...
### 🏗️ 架构特色
- ⚡ **现代化依赖注入**：`DatabaseDep`、`CacheDep` 类型别名
- 🛡️ **分层异常处理**：`ValidationException`、`DataServiceException`、`AIServiceException`
- 🔧 **中间件系统**：请求日志、性能监控、安全头
- 🎯 **完整类型安全**：MyPy + TypeScript 双重保障
- 🤖 **LangGraph工作流**：模块化多Agent架构
- 🚀 **三级缓存策略**：Redis → PostgreSQL → AKShare
...
### ⚡ v1.8.0 - 性能优化
- 🧹 **代码清理**：移除小时线功能，10分钟线聚合优化
- 📊 **数据库优化**：简化表结构，提升查询性能
- 🔧 **兼容性修复**：Pandas FutureWarning 修复
