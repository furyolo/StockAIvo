...
### v1.8.0 (2025-07-22) - 性能优化与逻辑统一
- ⚡ **性能优化**: 大幅优化 `get_market_aware_current_date()` 函数调用，单次分析从9次调用减少到3次（67%性能提升）
- 🎯 **逻辑统一**: 统一周度数据完整性判断逻辑，确保数据获取与AI分析的时间范围完全一致
- 🔧 **智能日期计算**: 重写 `_get_target_friday_date()` 函数，基于实时市场状态智能判断周度最后交易日
- 🛠️ **代码质量**: 修复所有类型错误，提升代码质量和类型安全性
- 📈 **向后兼容**: 所有优化保持完全向后兼容，无破坏性变更
