...
#### 技术分析缓存策略（2025年10月更新）
- Redis 键命名：`technical_analysis:{ticker}:{market_aware_date}`，日期统一使用 `YYYYMMDD`，便于按自然日排序与批量清理。
- TTL 策略：交易时间内固定 180 秒；闭市后自动扩展至下一次开盘时间，异常情况下回退到 600 秒。
- 序列化内容：包含 `analysis_text`、`metrics`、`generated_at`（ISO8601）与 `agent_version`，反序列化时统一格式化。
- 降级处理：Redis 不可用或序列化失败时自动回退至直接调用 Agent，并记录降级日志，保障主流程可用性。
- 监控优化：命中日志以 UTC 文本展示生成时间（例如 `2025-10-09 06:57:07 UTC`），便于跨时区排查。
