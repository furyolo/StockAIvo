...
class TestAgentsIntegration:
    ...
    def setup_method(self):
        ...
        # 模拟raw_data格式
        self.mock_state = cast(GraphState, {
            'ticker': 'AAPL',
            ...
            'raw_data': {
                'daily_prices': {
                    ...
                    'index': [ts.strftime('%Y-%m-%d') for ts in test_df.index]
                },
                'weekly_prices': {
                    'data': test_df.values.tolist(),
                    'columns': test_df.columns.tolist(),
...
class TestTenMinuteLineIntegration:
    ...
    def setup_method(self):
        ...
        self.mock_state_with_tenmin = cast(GraphState, {
            'ticker': 'AAPL',
            'custom_date_range': None,
            'raw_data': {
                'daily_prices': {
                    ...
                    'index': [ts.strftime('%Y-%m-%d %H:%M:%S') for ts in self.tenmin_df.index]
                },
                'weekly_prices': {
                    'data': self.tenmin_df.values.tolist(),
                    'columns': self.tenmin_df.columns.tolist(),
...
class TestTenMinuteLineEdgeCases:
    ...
    def test_tenmin_data_empty_dataframe(self):
        ...
        empty_tenmin_state = cast(GraphState, {
            'ticker': 'TEST',
            'custom_date_range': None,
            'raw_data': {
                'tenmin_prices': {
    ...
    def test_tenmin_data_malformed(self):
        """测试10分钟线数据格式错误的情况"""
        malformed_state = cast(GraphState, {
            'ticker': 'TEST',
            'custom_date_range': None,
            'raw_data': {
                'tenmin_prices': {
        ...
        large_state = cast(GraphState, {
            ...
            'raw_data': {
                'tenmin_prices': {
                    ...
                    'index': [ts.strftime('%Y-%m-%d %H:%M:%S') for ts in large_df.index]
                }
            },
            'analysis_results': {},
            'final_report': '',
