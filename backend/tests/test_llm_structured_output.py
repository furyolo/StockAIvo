...
def test_structured_response_validation():
    ...
    # 测试无效方向
    invalid_direction_payload: dict[str, Any] = {
    ...
    with pytest.raises(ValueError):
        MockStockPredictionResult(**invalid_direction_payload)
    
    # 测试无效置信度
    invalid_confidence_payload: dict[str, Any] = {
        "prediction_probability": 0.5,
        "direction": "UP",
    ...
    with pytest.raises(ValueError):
        MockStockPredictionResult(**invalid_confidence_payload)
